  ...
  const faqs = [
    {
      ...
      answer: (
        <p>
          {t.faq1a}{" "}
          <button
            onClick={() => window.dispatchEvent(new CustomEvent("open-hero-modal"))}
            className="text-[#b4735a] underline"
          >
            Just click here.
          </button>
        </p>
  ...
  return (
    <section className="bg-[#fef9f1] px-6 md:px-24 py-28">
      ...
      <div className="mt-16 divide-y divide-[#ded8cd] border-t border-b border-[#ded8cd]">
        {faqs.map((item, i) => (
          <div key={i}>
            <button onClick={() => toggle(i)} className="w-full flex justify-between items-center py-8 text-left">
              <span className="text-lg font-semibold text-[#3c3d42]">{item.question}</span>
              <span className="text-2xl text-[#3c3d42]">{openIndex === i ? "−" : "+"}</span>
            </button>
            {openIndex === i && (
              <div className="pb-8 text-[#3c3d42] text-base font-extralight leading-relaxed">{item.answer}</div>
            )}
          </div>
        ))}
      </div>
