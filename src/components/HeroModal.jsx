...
function HeroModal({ isOpen, onClose }) {
  ...
  const handleCountryChange = (e) => {
    ...
    // Removed automatic setting of country code
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error during Firestore submission:', err);
      setErrorMsg('We could not register you at this moment. Please try again later.');
    }
  };
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black/80 z-50">
      <div className="bg-white rounded-3xl py-20 px-20 max-w-lg w-full shadow-lg relative z-50">
        <button
          className="absolute top-4 right-4 text-3xl text-black hover:opacity-70"
          onClick={onClose}
        >
          &times;
        </button>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex space-x-2 w-full">
              <div className="relative">
                ...
                >
                  <option value="" disabled hidden>+XX</option>
            ...
            <button type="submit" className="w-full bg-[#B8775D] text-white py-3 rounded-lg font-semibold hover:opacity-90 transition">
              Join Waitlist
            </button>
            {errorMsg && (
