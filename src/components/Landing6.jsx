...
const mediaLogos = ["/Forbes.svg", "/WSJ.svg", "/People.svg", "/TheEconomist.svg", "/NYTimes.svg", "/AXIOS.svg"];

const ScrollingRow = ({ children, reverse }) => {
  const ref = useRef();
  useEffect(() => {
    let start = null;
    const step = (timestamp) => {
      ...
      if (ref.current) {
        ...
        ref.current.scrollLeft = reverse
          ? ref.current.scrollWidth / 2 - scrollAmount
          : scrollAmount;
      }
      ...
      <div className="mt-20 space-y-20">
        <ScrollingRow reverse>
          {testimonials.map((text, i) => (
            <div key={i} className="border rounded-xl p-6 max-w-[360px] flex-shrink-0 text-left font-extralight text-sm leading-relaxed whitespace-normal">
              {text.split(/(\*{2}.*?\*{2})/g).map((part, j) =>
                part.startsWith("**") ? (
        ...
        <ScrollingRow>
          {mediaLogos.map((logo, i) => (
            <Image key={i} src={logo} alt="media logo" width={100} height={64} className="h-16" />
          ))}
        </ScrollingRow>
      </div>
    </section>
